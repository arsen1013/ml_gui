...
class UI(QMainWindow):
    def __init__(self):
        ...
        data = data_() #data_ class를 data에 선언 
        steps = add_steps()
    ...
    def fill_combo_box(self):
        ...
        self.dropcolumn.addItems(self.column_list) #filldetails에서 만든 column_list내용 삽입 
    ...
    def con_cat(self):
        ...
        self.filldetails() #변경된 것 적용


    def dropc(self):
        # selected는 함수 안 변수라 위에서 만든 selected 랑 관련 없다. 
        selected = self.dropcolumn.currentText()
        self.df =  data.drop_columns(self.df, selected) #data_visualize.py에서 만든 함수 호출. 선택한 컬럼 명 드랍시켜준다. 
    ...
    def fillme(self):
        ...
        if type !='object': 
            # object 가 아닌 경우만
            self.df[self.empty_column.currentText()] = data.fillmean(self.df,self.empty_column.currentText()) #비어있는 곳에 평균값으로 채우기 
            # print("not object")
            self.filldetails() #변경된 것 적용
        else:
    ...
    def fill_na(self):
        self.df[self.empty_column.currentText()] = data.fillna(self.df,self.empty_column.currentText()) #비어있는 곳에 평균값으로 채우기 
        self.filldetails() #변경된 것 적용
